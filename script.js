...
(() => {
  ...
  const defaultState = {
    ...
    failedCases: [],
    archive: [
      { from:"SYSTEM", title:"신규 조사자 등록", body:"전국 일기 협회 조사에서 해당 조사자의 이름을 기억합니다." },
      { from:"UNKNOWN", title:"발신자 미상", body:"수칙서는 한 장이 아닙니다. 세 사람은 한곳에 있지 않습니다." }
    ]
  };
  ...
  function normalizeState(value) {
    ...
    safe.caseMistakes = value.caseMistakes && typeof value.caseMistakes === "object" ? value.caseMistakes : {};
    safe.failedCases = Array.isArray(value.failedCases) ? value.failedCases : [];
  ...
  function renderCases() {
    const caseCards = Object.entries(GAME_DATA.cases).map(([id, item]) => {
      const failed = state.failedCases.includes(id);
      const mistakes = state.caseMistakes[id] || 0;
      return `
        <button class="case-button ${failed ? "case-failed" : ""}" type="button"
          data-case-id="${id}" ${failed ? "disabled" : ""}>
          <div class="card-header">
            <strong>${item.code}</strong>
            <span class="${failed ? "danger" : "muted"}">${failed ? "조사 실패 · 잠금" : `오답 ${mistakes}/2`}</span>
          </div>
          <p>${item.title}</p>
          <p class="muted">${item.summary}</p>
          ${failed ? '<div class="failure-notice">오답이 2회 누적되어 접근 권한이 말소되었습니다. 진행 상황 초기화 후 재도전할 수 있습니다.</div>' : ""}
        </button>
      `;
    }).join("");
  ...
  function openCase(caseId) {
    if (state.failedCases.includes(caseId)) {
      flashMessage("조사 실패로 잠긴 사건입니다.");
      return;
    }
  ...
  function choose(index) {
    ...
    if (choice.wrong) {
      const nextMistakes = (state.caseMistakes[currentCaseId] || 0) + 1;
      state.caseMistakes[currentCaseId] = nextMistakes;

      if (nextMistakes >= 2) {
        addUnique(state.failedCases, currentCaseId);
        saveState();

        $("storyLog").insertAdjacentHTML("beforeend", `
          <div class="failure-notice">
            <strong>조사 실패</strong><br>
            잘못된 판단이 2회 누적되었습니다. 이 사건의 접근 권한이 영구 잠금되었습니다.
          </div>
        `);
        $("choiceList").innerHTML =
          '<button id="returnButton" class="choice-button" type="button">[ 잠긴 조사 목록으로 돌아가기 ]</button>';
        $("returnButton").addEventListener("click", backToCases);
        renderStatus();
        flashMessage("조사 실패 · 사건 접근 권한 말소");
        return;
      }
    }
    ...
    $("characterList").innerHTML = GAME_DATA.characters.map((character) => {
      ...
      return `
        <article class="content-card ${found ? "" : "locked"}">
          ...
          <p>${found ? character.description : "탐사 중 해당 인물의 생존 신호를 확보해야 상세 정보가 열립니다."}</p>
      ...
      return `
        <article class="content-card rule-card ${found ? "" : "locked"}">
          ...
          ${found
            ...
            : "<p>탐사 중 해당 문서를 발견해야 내용을 열람할 수 있습니다.</p>"}
        </article>
      `;
    }).join("");
  }
  ...
  function renderStatus() {
    ...
    const activeMistakes = currentCaseId ? (state.caseMistakes[currentCaseId] || 0) : 0;
    $("mistakeStat").textContent = `${activeMistakes}/2`;
  ...
  function runFinalMission() {
    ...
    const result = trueEnding
      ? "수칙서 아홉 장을 순서대로 낭독하자 복도, 식당, 교실이 하나의 출구로 이어졌습니다. 호갱은 당신에게 안겨 눈물을 흘리고, 감자는 남은 식량을 셋으로 나누어 분배했으며, 귀차니즘은 애인에게 ‘곧 간다’고 문자를 보냈습니다. 세 사람은 모두 송환했습니다."
      : "출구는 열렸지만 세 사람의 목소리가 서로 다른 방향에서 들렸습니다. 당신은 한 사람도 포기하지 않으려 했지만 눈을 뜨니 당신 혼자만 바깥으로 돌아왔습니다.";
